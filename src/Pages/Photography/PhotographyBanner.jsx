...
const PhotographyBanner = () => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const res = await fetch(`${API_BASE}/photographybanner`);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/photographybanner/upload`, {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex-1">
            <input
              ...
              className="border-2 border-dashed cursor-pointer border-gray-300 p-4 rounded-lg w-full"
            />
          </div>
          ...
          <button
            ...
            className={`px-6 py-3 rounded-md cursor-pointer text-white font-semibold transition
        ${
          uploading
