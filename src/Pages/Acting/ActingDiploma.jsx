...
const ActingDiploma = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      {/* Global PDF Section */}
      <div className="bg-white shadow-lg rounded-2xl p-6 mb-10 border border-indigo-200">
        <h3 className="text-xl font-semibold mb-4 text-indigo-700">Global Diploma PDF</h3>
        <input
          ref={fileInputRef}
          type="file"
          name="pdf"
        ...
        />
        {pdfFile && <p className="text-gray-600 mb-3 text-sm">Selected: {pdfFile.name}</p>}
        ...
        {globalPdf && (
          <a href={globalPdf} target="_blank" rel="noreferrer" className="text-blue-600 underline text-sm">
            View Global PDF
          </a>
        )}
      </div>
      ...
      <div className="bg-white shadow-lg rounded-2xl p-6 mb-10 border border-indigo-200">
        <input
          ...
          className="border-2 border-indigo-300 rounded-lg p-3 w-full mb-5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200"
        />
        {children.map((child, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row items-center gap-3 mb-3">
            <input
              ...
              className="border-2 border-indigo-300 rounded-lg p-3 flex-1 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200"
            />
            <button
              onClick={() => removeChild(idx)}
              className="bg-red-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-red-600 w-full sm:w-auto"
            >
              Remove
            </button>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            onClick={addChild}
            className="bg-blue-500 text-white px-5 py-2 rounded-md font-semibold hover:bg-blue-600 w-full sm:w-auto"
          >
            + Add Child
          </button>
          <button
            onClick={handleSave}
            className="bg-green-600 text-white px-5 py-2 rounded-md font-semibold hover:bg-green-700 w-full sm:w-auto"
          >
      ...
      {/* List */}
      <h3 className="text-2xl font-semibold mb-6 text-indigo-700">📌 Existing Entries</h3>
      {contents.length === 0 ? (
        <p className="text-gray-500">No entries added yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {contents.map((c) => (
            <div key={c.id} className="bg-white shadow-lg border border-indigo-200 rounded-2xl p-5 flex flex-col">
              <h4 className="font-bold text-indigo-700 text-lg mb-3">{c.title}</h4>
              <ul className="list-disc pl-5 flex-grow text-sm text-gray-700 space-y-1">
              ...
              <div className="flex gap-3 mt-5">
                <button
                  ...
                  className="flex-1 bg-yellow-400 text-black px-4 py-2 rounded-md font-semibold hover:bg-yellow-500"
                >
                  Edit
                </button>
                <button
