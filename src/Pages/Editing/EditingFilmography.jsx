...
const EditingFilmography = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/editingfilmography")
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/editingfilmography/upload", formData, {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/editingfilmography/${id}`);
