...
const CinematographyBanner = () => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const res = await fetch(`${API_BASE}/cinematographybanner`);
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`${API_BASE}/cinematographybanner/${id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold mb-4">Manage Cinematography Banners</h2>
