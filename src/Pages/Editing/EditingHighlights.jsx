...
export default function EditingHighlights() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API_BASE}/editinghighlights`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post(`${API_BASE}/editinghighlights/upload`, formData, {
  ...
  const handleDelete = async (_id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/editinghighlights/${_id}`);
      fetchItems();
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      ...
      >
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full py-3 rounded-md text-white cursor-pointer font-semibold transition ${
            loading
