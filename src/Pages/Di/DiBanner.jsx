...
const DiBanner = () => {
  ...
  const fetchBanners = async () => {
    try {
      const res = await axios.get("http://localhost:5000/dibanner/");
      setBanners(res.data);
    } catch (err) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/dibanner/upload", formData, {
  ...
  const handleDelete = async (fileName) => {
    try {
      await axios.delete(`http://localhost:5000/dibanner/${fileName}`);
      fetchBanners();
    } catch (err) {
