...
const EditingBanner = () => {
  ...
  const fetchBanners = async () => {
    try {
      const res = await axios.get("http://localhost:5000/editingbanner/");
      setBanners(res.data);
    } catch (err) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/editingbanner/upload", formData, {
  ...
  const handleDelete = async (fileName) => {
    try {
      await axios.delete(`http://localhost:5000/editingbanner/${fileName}`);
      fetchBanners();
    } catch (err) {
