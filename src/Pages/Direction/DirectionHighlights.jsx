...
export default function DirectionHighlights() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API_BASE}/directionhighlights`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleDelete = async (_id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/directionhighlights/${_id}`);
      fetchItems();
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      ...
      >
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full py-3 cursor-pointer rounded-md text-white font-semibold transition ${
            loading
