  ...
  const deletePdf = async () => {
    try {
      ...
      if (err.response?.status === 404) {
        alert(
          "PDF delete endpoint not found. Please check your backend routing configuration."
        );
      } else {
        alert(
          `Error deleting PDF: ${err.response?.data?.message || err.message}`
        );
      }
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="bg-white shadow-lg rounded-2xl p-6 mb-10 border border-indigo-200">
        ...
        />
        {pdfFile && (
          <p className="text-gray-600 mb-3 text-sm">Selected: {pdfFile.name}</p>
        )}
      ...
      {contents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {contents.map((c) => (
            <div
              key={c.id}
              className="bg-white shadow-lg border border-indigo-200 rounded-2xl p-5 flex flex-col"
            >
              <h4 className="font-bold text-indigo-700 text-lg mb-3">
                {c.title}
              </h4>
