...
const HomeFilmography = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/homefilmography")
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/homefilmography/upload", formData, {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/homefilmography/${id}`);
