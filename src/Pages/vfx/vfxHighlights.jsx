...
export default function VfxHighlights() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get("http://localhost:5000/vfxhighlights");
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/vfxhighlights", formData);
      setImage(null);
      setTitleLine("");
      fetchItems();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/vfxhighlights/${id}`);
      fetchItems();
    } catch (err) {
