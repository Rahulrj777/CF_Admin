...
export default function DiHighlights() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get("http://localhost:5000/dihighlights");
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/dihighlights", formData);
      setImage(null);
      setTitleLine("");
      fetchItems();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/dihighlights/${id}`);
      fetchItems();
    } catch (err) {
