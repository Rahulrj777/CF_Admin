      ...
      {months.map((month, mi) => (
        <div key={mi} className="border p-4 mb-6 rounded-md bg-gray-800">
          <div className="flex justify-between items-center mb-2">
            ...
            />
            <button onClick={() => addSection(mi)} className="ml-2 px-2 py-1 bg-blue-500 rounded">
              + Add Section
            </button>
          </div>
          ...
          {month.sections.map((section, si) => (
            <div key={si} className="ml-4 mb-4 border-l pl-4 border-gray-600">
              <div className="flex justify-between items-center mb-2">
                ...
                />
                <button onClick={() => deleteSection(mi, si)} className="ml-2 px-2 py-1 bg-red-500 rounded">
                  🗑️
                </button>
              </div>
              ...
              {section.items.map((item, ii) => (
                <div key={ii} className="flex items-center mb-2 ml-6 gap-2">
                  ...
                  />
                  <button onClick={() => deleteItem(mi, si, ii)} className="px-2 py-1 bg-red-500 rounded">
                    🗑️
                  </button>
                </div>
              ))}

              <button onClick={() => addItem(mi, si)} className="ml-6 px-2 py-1 bg-green-500 rounded">
                + Add Item
              </button>
            </div>
          ))}
...
{!pdf && !savedPdf && (
  ...
  />
      ...
      <div className="flex gap-4 mt-6">
        {!editMode && (
          <button onClick={addMonth} className="px-4 py-2 bg-purple-600 rounded" disabled={saving}>
            + Add Month
          </button>
        )}
        <button onClick={saveData} className="px-4 py-2 bg-green-600 rounded" disabled={saving}>
